...
END:VCARD
    ...
    conn.reply = (jid, text = '', quoted, options) => {
        return Buffer.isBuffer(text) ? this.sendFile(jid, text, 'file', '', quoted, false, options) : conn.sendMessage(jid, { ...options, jpegThumbnail: fs.readFileSync('./media/esce.jpg'), text }, { quoted, ...options })
    }
...
return await conn.sendMessage(jid, buttonMessage, { quoted: q })
  ...
  conn.sendButtonImg = async(jid, buffer, content, footer, button1, row1, q) => {
     ...
     let gbb = ob[Math.floor(Math.random() * ob.length)]
	  const buttons = [
	  {buttonId: row1, buttonText: {displayText: button1}, type: 1}
	  ]
const buttonMessage = {
    image: {url: buffer},
    jpegThumbnail: fs.readFileSync(`./media/${gbb}.jpg`),
    caption: content,
