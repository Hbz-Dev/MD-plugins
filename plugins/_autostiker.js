...
handler.before = async function (m) {
    ...
    if (chat.stiker && !user.banned && !chat.isBanned && !m.fromMe && !m.isBaileys) {
        ...
        if (/image/.test(mime)) {
            ...
            if (!img) return
            await conn.sendImageAsSticker(m.chat, img, m, { packname: global.packname, author: global.author, fileLength: 100 })
        } else if (/video/.test(mime)) {
            if ((q.msg || q).seconds > 11) return m.reply('Maksimal 10 detik!')
            let img = await q.download()
            if (!img) return
            await conn.sendVideoAsSticker(m.chat, img, m, { packname: global.packname, author: global.author, fileLength: 100 })
