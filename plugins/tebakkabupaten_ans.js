...
handler.before = async function (m) {
    let id = m.chat
    if (!m.quoted || !m.quoted.fromMe || !m.quoted.isBaileys || !/Ketik.*tekab/i.test(m.quoted.contentText)) return !0
    ...
    if (m.quoted.id == this.tebakkabupaten[id][0].id) {
        ...
        if (['.tekab', 'Bantuan', ''].includes(m.text)) return !0
        if (m.text.toLowerCase() == json.title.toLowerCase().trim()) {
            global.db.data.users[m.sender].exp += this.tebakkabupaten[id][2]
            await this.sendButton(m.chat, `*Benar!* +${this.tebakkabupaten[id][2]} XP`, 'Play Again? [LIMIT]', 'Tebak Kabupaten', '.tebakkabupaten', m)
