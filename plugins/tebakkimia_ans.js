...
handler.before = async function (m) {
    ...
    if (!(id in this.tebakkimia)) return m.reply('Soal Tebakkimia itu telah berakhir')
    if (m.quoted.id == this.tebakkimia[id][0].id) {
        ...
        if (m.text.toLowerCase() == json.unsur.toLowerCase().trim()) {
            ...
            await this.sendButton(m.chat, `*Benar!* +${this.tebakkimia[id][2]} XP`, 'Play Again? [LIMIT]', 'Tebak Kimia', '.tebakkimia', m)
